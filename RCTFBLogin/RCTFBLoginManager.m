...
- (NSDictionary *)constantsToExport {
  return @{
    ...
    @"LoginBehaviors": @{
      @"Web": [NSNumber numberWithInt:FBSDKLoginBehaviorBrowser],
      @"Browser": [NSNumber numberWithInt:FBSDKLoginBehaviorBrowser],
      @"Native": [NSNumber numberWithInt:FBSDKLoginBehaviorBrowser],
      @"SystemAccount": [NSNumber numberWithInt:FBSDKLoginBehaviorBrowser]
    }
  };
}
...
RCT_EXPORT_METHOD(loginWithPermissions:(NSArray *)permissions callback:(RCTResponseSenderBlock)callback) {
  ...
  [login logInWithPermissions:permissions fromViewController:nil handler:^(FBSDKLoginManagerLoginResult *result, NSError *error) {
    if (error) {
      [self fireEvent:@"Error" withData:@{
        @"description": error.localizedDescription
      }];
